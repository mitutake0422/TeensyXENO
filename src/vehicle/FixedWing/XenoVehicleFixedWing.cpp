...
void XenoVehicleFixedWing::InitVehicleValue()
{
	...
	m_stNavi.dAltP = 0;
	m_stNavi.dIAS = 0;
	m_stNavi.ushTcAltP = 9;
	m_stNavi.ushAltG = 100;
	m_stNavi.ushPressG = 313;
	m_stNavi.ushTemprG = 65;
	m_stNavi.dTcAltP = 1.0;
	m_stNavi.dAltG = 0.0;
	m_stNavi.dPressG = 1013.0;
	m_stNavi.dtemprG = 15.0;
	m_stNavi.dXTE = 0;
...
XenoVehicleType XenoVehicleFixedWing::GetVehicleType() const
{
	return XENO_VEHICLE_FIXEDWING;
}
			...
			if (s_nIdx == s_nSize + 8) {
				...
				if (ushCRC == ushTmp) {
					...
					if (uchMID == 0) {
						...
						m_core.m_stPlan.stWP[uchIdx].dWpAlt = (double)ulTmp / 1000.0 - 1000.0;
						...
						m_stSensor.dPhi = (double)ushTmp * 0.01 - 180.0;

						ushTmp = (((unsigned short)s_uchMsg[18] << 8) & 0xff00) | ((unsigned short)s_uchMsg[19] & 0x00ff);
						m_stSensor.dTht = (double)ushTmp * 0.01 - 180.0;
						...
						m_stSensor.dAlt = (double)ushTmp / 10.0 - 100.0;

						ushTmp = (((unsigned short)s_uchMsg[32] << 8) & 0xff00) | ((unsigned short)s_uchMsg[33] & 0x00ff);
						m_stSensor.dGS = (double)ushTmp * 0.01;

						ushTmp = (((unsigned short)s_uchMsg[34] << 8) & 0xff00) | ((unsigned short)s_uchMsg[35] & 0x00ff);
						m_stSensor.dCOG = (double)ushTmp * 0.01;

						if (m_stSensor.dCOG > 180.0) {
							m_stSensor.dCOG -= 360.0;
						}

						m_stSensor.uchHour = s_uchMsg[36];
						m_stSensor.uchMin = s_uchMsg[37];
						m_stSensor.uchSec = s_uchMsg[38];

						m_stSensor.uchNSV = s_uchMsg[39] & 0x0f;
						m_stSensor.uchQuality = (s_uchMsg[39] >> 4) & 0x0f;

						ushTmp = (((unsigned short)s_uchMsg[40] << 8) & 0xff00) | ((unsigned short)s_uchMsg[41] & 0x00ff);
						m_stSensor.dPs = (double)ushTmp / 10.0;	// [hPa]

						ushTmp = (((unsigned short)s_uchMsg[42] << 8) & 0xff00) | ((unsigned short)s_uchMsg[43] & 0x00ff);
						m_stSensor.dPd = (double)ushTmp / 10.0;	// [Pa]

						ushTmp = (((unsigned short)s_uchMsg[44] << 8) & 0xff00) | ((unsigned short)s_uchMsg[45] & 0x00ff);
						m_stSensor.dVS = (double)ushTmp / 100.0 - 100.0;	// [m/s]
					}
...
void XenoVehicleFixedWing::SendXenoController()
{
	...
	if (s_ushCnt % 2 == 0) {
		...
		uchMsg[11] = ((m_stSensor.uchQuality << 4) & 0xf0) | (m_stSensor.uchNSV & 0x0f);
		...
		uchMsg[14] = (unsigned char)m_core.m_stPlan.nWpNum;
		uchMsg[15] = (unsigned char)m_core.m_stPlan.nWpTo;
	...
	if (s_ushCnt % 10 == 0) {
		...
		uchMsg[3] = m_stSensor.uchHour;
		uchMsg[4] = m_stSensor.uchMin;
		uchMsg[5] = m_stSensor.uchSec;
